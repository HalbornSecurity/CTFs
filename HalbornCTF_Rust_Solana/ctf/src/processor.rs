...
pub fn create_game_config(
    ...
    invoke_signed(
        &create_account(
            ...
            Rent::get()?.minimum_balance(std::mem::size_of::<GameConfig>()),
            std::mem::size_of::<GameConfig>().try_into().map_err(|_| ProgramError::MaxAccountsDataSizeExceeded)?,
            &crate::id()
        ),
        &[
            admin_info.clone(),
...
pub fn create_user(
    ...
    invoke_signed(
        &create_account(
            authority_info.key,
            user_info.key,
            Rent::get()?.minimum_balance(std::mem::size_of::<User>()),
            std::mem::size_of::<User>().try_into().map_err(|_| ProgramError::MaxAccountsDataSizeExceeded)?,
            &crate::id()
        ),
        &[
